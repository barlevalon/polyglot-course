...
func getArgs(opts *Opts) ([]string, error) {
	...
	if operation == Add {
		if len(opts.Args) != 3 {
			return nil, fmt.Errorf("add requires 2 arguments, but receieved %v", len(opts.Args))
		}
		return opts.Args[1:], nil
	}
	if operation == Remove {
		if len(opts.Args) != 2 {
			return nil, fmt.Errorf("add requires 1 arguments, but receieved %v", len(opts.Args))
		}
		return opts.Args[1:], nil
	}

	if len(opts.Args) > 1 {
		return nil, fmt.Errorf("print expects 0 or 1 arguments, but receieved %v", len(opts.Args))
	}
